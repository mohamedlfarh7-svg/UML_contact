...
<head>
    ...
    <title>Luxe | Ajouter un Parfum</title>
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-6 md:p-12 flex flex-col items-center">
            <header class="w-full max-w-2xl mb-10">
                <h1 class="text-4xl font-black tracking-tighter italic uppercase text-[#1B2559]">Ajouter un Produit</h1>
                <p class="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mt-2">Nouvelle Fragrance Luxe</p>
            </header>

            <div class="w-full max-w-2xl bg-white rounded-[3rem] p-10 shadow-[0_20px_50px_rgba(0,0,0,0.02)] border border-slate-50">
                <form action="{{ route('parfums.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Nom du Parfum</label>
                        <input type="text" name="name" class="w-full px-8 py-5 bg-slate-50 border-none rounded-[1.5rem] focus:ring-2 focus:ring-blue-600/10 transition-all font-bold text-sm" placeholder="Ex: Royal Oud" required>
                    </div>

                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Prix (DH)</label>
                            <input type="number" name="price" class="w-full px-8 py-5 bg-slate-50 border-none rounded-[1.5rem] focus:ring-2 focus:ring-blue-600/10 transition-all font-bold text-sm" placeholder="1200" required>
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Stock Initial</label>
                            <input type="number" name="quantity" class="w-full px-8 py-5 bg-slate-50 border-none rounded-[1.5rem] focus:ring-2 focus:ring-blue-600/10 transition-all font-bold text-sm" placeholder="20" required>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Visuel Produit</label>
                        <div class="relative group">
                            <div class="border-2 border-dashed border-slate-100 rounded-[2rem] p-12 text-center group-hover:border-blue-200 transition-all bg-slate-50/50">
                                <i class="ph ph-image-square text-4xl text-slate-200 mb-4 block mx-auto"></i>
                                <span class="text-[10px] text-slate-400 font-black uppercase tracking-widest">Glisser ou cliquer pour uploader</span>
                            </div>
